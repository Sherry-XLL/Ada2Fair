...
class TopkMetric(AbstractMetric):
    ...
    def topk_result(self, metric, value):
        ...
        std_result = value.std(axis=0)
        var_result = value.var(axis=0)
        for k in self.topk:
            key = "{}@{}".format(metric, k)
            metric_dict[key] = round(avg_result[k - 1], self.decimal_place)
            metric_dict["{}@{}".format(metric + "_Std", k)] = round(
                std_result[k - 1], self.decimal_place
            )
            metric_dict["{}@{}".format(metric + "_Var", k)] = round(
                var_result[k - 1], self.decimal_place
            )
        return metric_dict
